...
function Root() {
  ...
  useEffect(() => {
    if (isWidget) return;
    getSession().then(async (session) => { if (session) return setUser(session); const last = localStorage.getItem("memo-agent-last-account"); setUser(last ? await enterAccount(last).catch(() => null) : null); }).catch(() => setUser(null));
  }, [isWidget]);
