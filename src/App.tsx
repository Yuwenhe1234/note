      ...
      setTasks([
        {
          ...
          mindMap: analysis?.flowchart?.nodes.length ? { nodes: analysis.flowchart.nodes.map((node, index) => ({ id: node.id, label: node.label, x: 80 + (index % 3) * 220, y: 70 + Math.floor(index / 3) * 150 })), edges: analysis.flowchart.edges.map((edge, index) => ({ id: `flow-${index}`, source: edge.from, target: edge.to })) } : undefined,
