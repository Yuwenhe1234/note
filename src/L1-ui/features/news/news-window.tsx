  ...
  return <section className="feature-window news-window">
    ...
    <div className="news-layout">
      <div className="news-sidebar" data-testid="news-sidebar">
        <aside className="news-sources"><h2>订阅来源</h2><label><span>订阅链接</span><input aria-label="订阅链接" value={url} onChange={(event) => setUrl(event.target.value)} onKeyDown={(event) => event.key === "Enter" && add()} placeholder="粘贴账号主页或网站链接" /></label><button className="news-add" onClick={add}><Plus />添加来源</button>
          <div className="news-source-list news-source-scroll" role="list" aria-label="订阅来源列表">{state.sources.map((source) => <article role="listitem" key={source.id}><div><strong>{source.name}</strong><span>{source.platform} · {source.loginStatus === "required" ? "需要重新登录" : "已订阅"}</span></div>{source.platform !== "website" && <button aria-label={`登录 ${source.name}`} onClick={async () => { try { await loginRequest(source.url); repo.updateSource(source.id, { loginStatus: "connected" }); setNotice("已打开 Edge 登录窗口，完成登录后再点击刷新"); reload(); } catch (error) { setNotice(error instanceof Error ? error.message : "登录窗口打开失败"); } }}><LogIn /></button>}<button aria-label={`删除 ${source.name}`} onClick={() => { repo.removeSource(source.id); reload(); }}><Trash2 /></button></article>)}</div>
        </aside>
    ...
    {selectedItem && <div className="news-detail-overlay" data-testid="news-detail-overlay" onMouseDown={(event) => { if (event.target === event.currentTarget) closeDetail(); }}>
      <section className="news-detail-modal" role="dialog" aria-modal="true" aria-labelledby="news-detail-title">
        ...
        <div className="news-detail-heading"><small>UP 主</small><strong>{selectedItem.sourceName}</strong><small>视频标题</small><h2 id="news-detail-title">{selectedItem.title}</h2></div>
