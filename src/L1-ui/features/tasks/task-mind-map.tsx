...
export function TaskMindMap({ value, onChange, className = "", onDoubleClick }: { value: TaskMindMap; onChange: (value: TaskMindMap) => void; className?: string; onDoubleClick?: () => void }) {
  ...
  const handleNodeDoubleClick = (event: MouseEvent, node: Node) => { event.stopPropagation(); if (onDoubleClick) { onDoubleClick(); return; } setNodes((current) => current.map((item) => item.id === node.id ? { ...item, data: { ...item.data, editing: true, onRename: (label: string) => setNodes((latest) => latest.map((entry) => entry.id === node.id ? { ...entry, data: { ...entry.data, label: label.trim() || entry.data.label, editing: false } } : entry)) } } : item)); };
