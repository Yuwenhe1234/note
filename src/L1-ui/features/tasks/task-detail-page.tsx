...
export function TaskDetailPage({ task, onBack, onSave, onRegenerate }: { task: Task; onBack: () => void; onSave: (task: Task) => void; onRegenerate?: () => void }) {
  ...
  return <section className="task-detail-page">
    ...
    <footer className="task-detail-footer">{onRegenerate ? <button className="btn-secondary" onClick={onRegenerate}><RefreshCw /> 让 AI 重新生成</button> : <button className="btn-secondary" onClick={onBack}>取消</button>}<button className="primary" onClick={() => onSave(draft)}><Check /> 保存任务</button></footer>
