...
export function TaskDetailPage({ task, onBack, onSave }: { task: Task; onBack: () => void; onSave: (task: Task) => void; onRegenerate?: () => void }) {
  const [draft, setDraft] = useState(task);
  const [mindMapOpen, setMindMapOpen] = useState(false);
  const [titleLimitFlash, setTitleLimitFlash] = useState(false);
  const flashTitleLimit = () => { setTitleLimitFlash(true); window.setTimeout(() => setTitleLimitFlash(false), 420); };
  ...
  return <section className="task-detail-page">
    <div className="task-detail-topbar"><button className="task-detail-back" aria-label="返回任务清单" onClick={onBack}><ArrowLeft /></button><header className="task-detail-header">
      <div className="task-detail-title"><div>{draft.type ? <span className="task-detail-type">{draft.type}</span> : null}<textarea rows={2} maxLength={32} className={`task-title-input ${draft.title.length > 18 ? "is-long" : ""} ${titleLimitFlash ? "limit-flash" : ""}`} aria-label="任务名称" onKeyDown={(event) => { if (draft.title.length >= 32 && event.key.length === 1) flashTitleLimit(); }} value={draft.title} onChange={(event) => setDraft({ ...draft, title: event.target.value })} /></div><div className="task-detail-objective"><small>完成目标</small><textarea aria-label="完成目标" value={draft.objective || draft.goal} onChange={(event) => setDraft({ ...draft, objective: event.target.value, goal: event.target.value })} /></div></div>
    </header></div>
    <main className="task-detail-grid">
      <aside className="task-detail-left">{mindMapOpen ? <div className="task-mind-map" aria-hidden="true" /> : <TaskMindMap value={draft.mindMap || { nodes: [], edges: [] }} onChange={(mindMap) => setDraft((current) => ({ ...current, mindMap }))} onDoubleClick={() => setMindMapOpen(true)} />}<section><h2>推荐学习资源</h2>{draft.resourceRecords?.map((resource) => <div className="task-detail-resource" key={`${resource.platform}-${resource.name}`}><small>{resource.platform} · {resource.stage}</small><strong>{resource.name}</strong><span>{resource.reason}</span>{resource.url ? <a href={resource.url} target="_blank" rel="noreferrer">打开资源</a> : <em>搜索：{resource.searchQuery}</em>}</div>)}<ResourceGroup label="B站" value={resources.externalRecommendations.upMasters} onChange={(upMasters) => setDraft({ ...draft, resources: { ...resources, externalRecommendations: { ...resources.externalRecommendations, upMasters } } })} /><ResourceGroup label="抖音" value={resources.externalRecommendations.communities} onChange={(communities) => setDraft({ ...draft, resources: { ...resources, externalRecommendations: { ...resources.externalRecommendations, communities } } })} /><ResourceGroup label="网站" value={resources.externalRecommendations.websites} onChange={(websites) => setDraft({ ...draft, resources: { ...resources, externalRecommendations: { ...resources.externalRecommendations, websites } } })} /></section></aside>
    ...
    <footer className="task-detail-footer"><button className="btn-secondary" onClick={onBack}>取消</button><button className="primary" onClick={() => onSave(draft)}><Check /> 保存任务</button></footer>
    {mindMapOpen && <div className="mind-map-overlay" role="dialog" aria-label="完整思维导图"><button className="mind-map-close" aria-label="关闭完整思维导图" onClick={() => setMindMapOpen(false)}><X /></button><TaskMindMap className="task-mind-map-fullscreen" value={draft.mindMap || { nodes: [], edges: [] }} onChange={(mindMap) => setDraft((current) => ({ ...current, mindMap }))} /></div>}
  </section>;
}
