...
- **两阶段建任务**:先填基础信息,再触发 AI 分析；分析完成后直接进入可编辑任务详情预览，保存或取消。
- **步骤管理**:子任务以 0.25 小时为刻度、可勾选完成气泡展示,支持双击改时长、拖动排序、增删步骤。
- **今日待办**:手动添加、浏览器语音识别(中文时间解析)、AI 一键生成、定时网页提醒（网页打开时）。
- **AI 分析**:按学习、开发/项目、写作/内容、规划/准备、生活/习惯五类方法论生成任务类型、可验证目标、具体步骤、注意事项、资源建议与知识地图；服务端严格校验返回结构并自动重试。
- **任务知识地图**:AI 将任务主题、一级模块和二级内容生成树状思维导图；支持框选、拖动、连线、撤回、创建主题/子节点和编辑节点。
...
公开地址：<https://yuwenhe1234.github.io/note/>

网页版无需登录，任务、待办、设置与消息数据保存在当前浏览器。更换设备、使用无痕窗口或清理站点数据不会自动迁移数据，请定期在“设置 → 数据与备份”中导出备份。

AI 功能使用每位访客自己填写的 API Key。Key 只保存在当前浏览器，不会提交到 GitHub；任务内容会在用户主动调用 AI 时发送给所选服务商。部分服务商可能不允许浏览器跨域直连，此时应用会提示更换兼容接口。

“今日消息”在网页版采用手动模式：先添加订阅来源，再粘贴内容链接、标题或正文，可选择使用自己的 AI 配置生成摘要。自动抓取、平台登录、定时刷新和桌面小组件仍属于 Tauri 桌面版能力。
...
该命令使用 GitHub Pages 项目路径 `/note/`。仓库的 Pages 发布来源需要设置为 **GitHub Actions**；推送到 `main` 后，工作流会依次执行类型检查、测试、构建和部署。
...
配置后可点击 **测试当前连接** 验证。AI 分析会校验返回的 JSON 结构：任务类型、结构化目标、知识地图、资源、注意事项、步骤和时长必须完整；步骤时长以 `0.25` 小时为刻度，`estimatedHours` 必须等于各步骤时长之和。格式不符时自动重试一次。

AI 资源建议覆盖 Bilibili、抖音、官方文档、网站课程、GitHub 和社区。没有确认真实存在的链接时，系统保留搜索关键词，不编造 URL。
...
今日待办的到点提醒在网页内以提醒窗口呈现。桌面版“发送测试通知”会调用系统通知并弹出提醒窗口；网页关闭或设备休眠期间不保证触发。AI 陪伴后续可订阅同一提醒事件，当前不提供 AI 陪伴提醒。
